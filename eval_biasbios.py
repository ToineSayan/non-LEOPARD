...
def eval_biasbios(X_erased, Z, Y, X_test_erased, Z_test, Y_test, name, output_path):
    ...
    score_train, score_test, std_train, std_test, clfs = eval_clf(
        ...
        n_eval = n_eval, 
        save = True, 
        output_path_and_name = output_path + f'/clf_biasbios_y_{name}'
        )
    ...
    mdl = MDL(X_erased[:100000], Z[:100000], max_iter=20)
...
if __name__ == "__main__":
    ...
    X, Z, Y, _, _, _, X_test, Z_test, Y_test, _, _ = load_data("biasbios")


    import warnings
    with warnings.catch_warnings(action="ignore"):
        eval_biasbios(X, Z, Y, X_test, Z_test, Y_test, "orig_final", "./outputs")